...
<section class="create-edit-page">
	...
	<form action="{{ route('pages.update', $page->slug) }}" name="createPage" method="POST" enctype="multipart/form-data">
		{{ method_field('PUT') }}
		{!! csrf_field() !!}
				...
				<div class="row">
					<div class="small-6 columns">
						...
						<div class="row">
							...
							<div class="small-7 float-left columns">
								<select id="country" name="country">
									...
									<option value="{{ $country->id }}" {{ (old('country', $page->country_id) == $country->id) ? 'selected' : '' }}>{{ $country->name }}</option>
									@endforeach
								</select>
						...
						<div class="row" id="region-row">
							...
							<div class="small-7 float-left columns">
								<select id="region" name="region">
									<option value="">Select...</option>
									@if((old('country', $page->country_id) && count($regions) > 0))
									@foreach($regions as $region)
									<option value="{{ $region->id }}" {{ (old('region', $page->region_id) == $region->id) ? 'selected' : '' }}>{{ $region->name }}</option>
									@endforeach
									@endif
								</select>
						...
						<div class="row">
							...
							<div class="small-7 float-left columns">
								<select id="city" name="city">
									<option value="">Select...</option>
									@if(old('region', $page->region_id) && count($cities) > 0)
									@foreach($cities as $city)
									<option value="{{ $city->id }}" {{ (old('city', $page->city_id) == $city->id) ? 'selected' : '' }}>{{ $city->name }}</option>
									@endforeach
									@endif
								</select>
						...
						<div class="row">
							...
							<div class="small-7 float-left columns">
								<input id="address" type="text" name="address" value="{{ old('address', $page->address) }}">
						...
						<div class="row">
							...
							<div class="small-7 float-left columns">
								<input id="zip-code" type="text" name="zip_code" value="{{ old('zip_code', $page->zip_code) }}">
					...
					<div class="small-5 float-right columns">
						<div class="row">
							...
							<div class="small-8 columns">
								<select id="type" name="type">
									<option value="">Select...</option>
									@if(isset($attributes['page.type_group']))
									@foreach($attributes['page.type_group'] as $type_group)
									<optgroup label="{{ $type_group->name }}">
										@foreach($type_group->children as $type)
										<option value="{{ $type->id }}" {{ ($type->id == old('type', $page->attributes['page.type'][0]->id)) ? 'selected' : '' }}>{{ $type->name }}</option>
										@endforeach
									</optgroup>
									@endforeach
									@endif
						...
						<div class="row">
							...
							<div class="small-8 float-left columns">
								<textarea id="status" name="status" rows="4">{{ old('status', $page->status) }}</textarea>
							</div>
						</div>
					</div>
				</div>
			...
			<div class="small-11 small-centered columns">
				<div class="row">
					...
					<div class="small-6 float-left columns">
						<textarea id="story" name="story" rows="8">{{ old('story', $page->story) }}</textarea>
		...
		<div class="gray-block">
					...
					<div class="row">
						...
						<div class="small-7 columns">
							<select id="year-founded" name="year_founded">
								...
								<option value="{{ $year->id }}" {{ (isset($page->attributes['page.year']) && $year->id == old('year_founded', $page->attributes['page.year'][0]->id)) ? 'selected' : '' }}>{{ $year->name }}</option>
								@endforeach
								@endif
							</select>
					...
					<div class="row">
						...
						<div class="small-7 columns">
							<select id="activity-period" name="activity_period">
								...
								<option value="{{ $activity_period->id }}" {{ (isset($page->attributes['page.activity_period']) && $activity_period->id == old('activity_period', $page->attributes['page.activity_period'][0]->id)) ? 'selected' : '' }}>{{ $activity_period->name }}</option>
								@endforeach
								@endif
							</select>
					...
					<div class="row">
						...
						<div class="small-7 columns">
							<select id="season" name="season">
								...
								<option value="{{ $season->id }}" {{ (isset($page->attributes['page.season']) && $season->id == old('season', $page->attributes['page.season'][0]->id)) ? 'selected' : '' }}>{{ $season->name }}</option>
								@endforeach
								@endif
							</select>
			...
			<div class="row">
				<div class="small-12 columns" id="selected-key-people">
					@if($page->keyPeople)
					@foreach($page->keyPeople as $person)
...
@if(Session::has('edit_page_main_image'))
<?php $oldMainImage = Session::get('edit_page_main_image'); ?>
<script type="text/javascript">
	var oldMainImage = {!! json_encode($oldMainImage) !!};
</script>
@endif

@if(Session::has('edit_page_bg_image'))
<?php $oldBgImage = Session::get('edit_page_bg_image'); ?>
<script type="text/javascript">
...
<script>
	var pageID            = {!! json_encode($page->id) !!}
	var pageSlug          = {!! json_encode($page->slug) !!}
	
	Dropzone.autoDiscover = false;

	var pageMainImage = new Dropzone('#upload-main-image', {
		url: "<?php echo route('pages.create.ajaxUploadImage'); ?>" + "?page=" + pageSlug,
		...
		init: function() {
			...
			this.on('removedfile', function(file){
				if(file.status !== "error"){
					$.ajax({
						type: "DELETE",
						url: "<?php echo route('pages.create.ajaxDeleteImage'); ?>" + '?page=' + pageSlug + '&image=' + file.newFileName,
			...
			if(typeof oldMainImage !== 'undefined'){
				...
				var filePath = "/uploads/pages/" + pageID + '/images/' + file.name;
				if(oldMainImage['is_temp'] == 1){
