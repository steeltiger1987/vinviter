...
@section('title', 'Upcoming admin events - Dashboard')
    ...
    <div class="small-12 columns events my-events">
        <div class="row">
            ...
            <ul class="menu event-type-mode-menu float-right">
                    <li><a href="{{ route('dashboard.upcomingEvents') }}">Creator</a></li>
                    <li class="active"><a href="{{ route('dashboard.upcomingEvents.typeAdmin') }}">Admin</a></li>
                </ul>
            </div>
        </div>

        @forelse($events as $event)
        @include('dashboard.adminUpcomingEvent_row')
        @empty
...
<div class="small-12 columns">
        <button class="dashboard-events-load-more load-more" data-request-url="{{ route('dashboard.upcomingEvents.typeAdmin') }}" data-next-page="{{ $events->currentPage()+1 }}" data-last-page="{{ $events->lastPage() }}" data-left-off="{{ $events->last()->id }}"><span class="fa fa-refresh"></span>Load more</button>
