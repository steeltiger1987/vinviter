...
@section('title', 'History events - Dashboard')
        ...
        <div class="row">
            ...
            <div class="small-6 columns">
                <ul class="menu event-type-mode-menu float-right">
                    <li class="active"><a href="{{ route('dashboard.historyEvents') }}">Creator</a></li>
                    <li {{ (Auth::user()->adminOfEvents()->history()->count() > 0) ? '' : 'class=list-disabled' }}><a href="{{ route('dashboard.historyEvents.typeAdmin') }}">Admin</a></li>
                </ul>
            </div>
        </div>
        @forelse($events as $event)
        @include('dashboard.creatorHistoryEvent_row')
        @empty
...
<div class="row">
    <div class="small-12 columns">
        <button class="dashboard-events-load-more load-more" data-request-url="{{ route('dashboard.historyEvents') }}" data-next-page="{{ $events->currentPage()+1 }}" data-last-page="{{ $events->lastPage() }}" data-left-off="{{ $events->last()->id }}"><span class="fa fa-refresh"></span>Load more</button>
