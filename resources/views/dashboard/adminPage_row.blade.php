<div class="row page-block" data-id="{{ $page->id }}">
    <div class="small-10 columns">
        ...
        <div class="page-content float-left">
            <h4><a href="{{ route('pages.show', $page->slug) }}">{{ $page->name }} <span class="user-page-relation">Admin</span></a></h4>
            <div class="page-info">
    ...
    <a data-open="page-invite-{{ $page->id }}" class="button">Invite</a>
        ...
        <a data-open="remove-page-admin-{{ $page->id }}" class="button hollow">Remove</a>
    </div>
    ...
    <div class="reveal" id="remove-page-admin-{{ $page->id }}" data-reveal>
        <h4>Remove</h4>
        <p>Are you sure you want to remove yourself as admin from this page?</p>
        <p class="float-right remove-admin-buttons">
            <button type="button" class="button secondary" data-close>Cancel</button>
            <button type="button" class="button alert remove-page-admin-confirm" data-admin-id="{{ Auth::id() }}" data-slug="{{ $page->slug }}" data-id="{{ $page->id }}">Remove</button>
        </p>
